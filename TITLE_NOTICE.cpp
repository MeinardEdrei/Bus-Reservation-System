...
int main () // =============================================================================================== MAIN PROGRAM 
{
	...
	TITLE: // ============================================================================================== TITLE SCREEN
	{
		...
		for( int BORDER_X_axis = 34 ; BORDER_X_axis < 117 ; BORDER_X_axis++ )
		{
	...
	MOVING_BUS: // ========================================================================================= MOVING BUS ANIMATION
	{
		...
		gotoXY_BUS ( 5, 46 ) ; printf( "__________________         ___________________            __________________              __________________           _____________________" );
		...
		int X_ENTERaxis, Y_ENTERaxis; // =================================== DECLARATION FOR ENTER BORDER
		...
		gotoXY_STRING ( 50 , 27 );
			...
			for ( Y_ENTERaxis = 45 ; Y_ENTERaxis <= 101 ; Y_ENTERaxis++ ){ // up
			    
			    gotoXY_STRING ( Y_ENTERaxis , 33 ); printf( "%c" , 205 );
			    gotoXY_STRING ( Y_ENTERaxis , 37 ); printf( "%c" , 205 );
			    gotoXY_STRING ( Y_ENTERaxis , 38 ); printf( "%c" , 205 );
		    }
		    
		    for ( X_ENTERaxis = 34 ; X_ENTERaxis <= 37 ; X_ENTERaxis++ ){
		    	
		    	gotoXY_STRING ( 45 , X_ENTERaxis ); printf( "%c" , 186 ); // side left
		    	gotoXY_STRING ( 102 , X_ENTERaxis ); printf( "%c" , 186 ); // side right
		    	
		    	// EDGES
		    	gotoXY_STRING ( 45 , 38 ); printf( "%c" , 200 ); // corner left
		    	gotoXY_STRING ( 45 , 33 ); printf( "%c" , 201 ); // corner left
		    	gotoXY_STRING ( 102 , 38 ); printf( "%c" , 188 ); // corner right
		    	gotoXY_STRING ( 102 , 33 ); printf( "%c" , 187 ); // corner right
			}
		    ...
		    Color(15); gotoXY_STRING ( 53 , 35 ); printf( "   >>>>----      E N T E R      ----<<<< " );
		    
			cin.ignore(std::numeric_limits<streamsize>::max(),'\n') ;
			gotoXY_STRING ( 50 , 27 );
			SetConsoleTextAttribute ( hConsole, 5 ); printf ( ".~.===: : : :|   ..===.                    .~.===: :");
		...
		for ( BUS_X_axis = 6 , BUS_Y_axis = 32 ; BUS_X_axis < 64 ; BUS_X_axis++ )  
		{
		   	gotoXY_BUS ( BUS_X_axis, 32 );
			   	SetConsoleTextAttribute ( hConsole, 7 ) ; printf( "      Bus is moving, mind the ", '\r');
			  	SetConsoleTextAttribute ( hConsole, 10 ) ; printf( "stoplight", '\r');
			  	SetConsoleTextAttribute ( hConsole, 0 ) ; printf( "!", 'r' );
			...
			if ( BUS_X_axis == 63 ) // end of loading
			{
				gotoXY_BUS ( BUS_X_axis, 32 );
					SetConsoleTextAttribute ( hConsole, 10 ) ; printf( "      We have arrived! ", '\r');
			   		SetConsoleTextAttribute ( hConsole, 7 ) ; printf( "Please proceed after 4 seconds...", '\r');
	...
	NOTICE: // ============================================================================================= NOTICE AFTER LOADING
	{
		...
		gotoXY_STRING ( 56 , 5 ) ; SetConsoleTextAttribute ( hConsole, 11 ) ; printf( "-     -     N O T I C E     -     -" );
		
		int BORDER_X_axis = 0, BORDER_Y_axis = 0;
		
		SetConsoleTextAttribute ( hConsole, 15 );
		
		for( BORDER_X_axis = 24 ; BORDER_X_axis < 125 ; BORDER_X_axis++ ) 
		{
			gotoXY_BORDER ( BORDER_X_axis, 6 ) ; printf( "%c", 196 ); 	// up
			gotoXY_BORDER ( BORDER_X_axis, 40 ) ; printf( "%c", 196 ); 	// down
		}
		
		for( BORDER_Y_axis = 6 ; BORDER_Y_axis < 41 ; BORDER_Y_axis++ )
		{
			gotoXY_BORDER ( 24 , BORDER_Y_axis ) ; printf( "%c", 179 ); 	// left side
			gotoXY_BORDER ( 124 , BORDER_Y_axis ) ; printf( "%c",179 );		// right side
			
			gotoXY_BORDER ( 24 , 6 ) ; printf( "%c", 218 );		// top left
			gotoXY_BORDER ( 124 , 6 ) ; printf( "%c", 191 );	// top right
			gotoXY_BORDER ( 24 , 40 ) ; printf( "%c", 192 );	// bottom left
			gotoXY_BORDER ( 124 , 40 ) ; printf( "%c", 217 );	// bottom right
		}
		...
		Sleep ( 1000 ); 	// 1 second delay
			...
			gotoXY_STRING ( 56 , 8 ) ; SetConsoleTextAttribute ( hConsole, 10 ) ; printf( "MNL GOJO BUSLINE | TICKETING SIMULATION" );
			gotoXY_STRING ( 37 , 10 ) ; SetConsoleTextAttribute ( hConsole, 15 ) ; printf( "A program dedicated for") ; SetConsoleTextAttribute ( hConsole, 13 ) ; printf ( " bus ticketing" );
			SetConsoleTextAttribute ( hConsole, 15 ) ; printf ( ". It's purpose is to allow the user to " );
			gotoXY_STRING ( 31 , 11 ) ; printf( "experience booking in a rather unique sequence. The program will ask you for mandatory " );
			gotoXY_STRING ( 40 , 12 ) ; printf( "aspects such as " ) ; SetConsoleTextAttribute ( hConsole, 11 ) ; printf( "Type of Bus, Destination, Passenger(s) Details, etc." );
			gotoXY_STRING ( 33 , 13 ) ; SetConsoleTextAttribute ( hConsole, 15 ) ; printf( "Within a few parts of the program, you may notice a few addons to make booking or " );
			gotoXY_STRING ( 40 , 14 ) ; printf( "reserving interesting.") ; SetConsoleTextAttribute ( hConsole, 6 ) ; printf( " There may also be a few easter eggs for fun." );
		
		Sleep ( 2200 ); 	// 2 second delay
		
			gotoXY_STRING ( 61 , 17 ) ; SetConsoleTextAttribute ( hConsole, 4 ) ; printf( "! FREQUENT BUG ENCOUNTERS !" );
			gotoXY_STRING ( 51 , 19 ) ; SetConsoleTextAttribute ( hConsole, 12 ) ; printf ( "Please AVOID clicking anything in the program." );
			gotoXY_STRING ( 42 , 20 ) ; SetConsoleTextAttribute ( hConsole, 15 ) ; printf ( "-> There will be instructions on specific parts as you go on." );
			gotoXY_STRING ( 62 , 22 ) ; SetConsoleTextAttribute ( hConsole, 12 ) ; printf ( "Do NOT resize the window." );
			gotoXY_STRING ( 46 , 23 ) ; SetConsoleTextAttribute ( hConsole, 15 ) ; printf ( "-> If the default window size does not fit the screen, " );
			gotoXY_STRING ( 38 , 24 ) ; printf ( "consider maximizing the window by pressing the box icon on the top right." );
			gotoXY_STRING ( 51 , 25 ) ; SetConsoleTextAttribute ( hConsole, 12 ) ; printf ( "PLEASE REFRAIN from making any further changes " );
			gotoXY_STRING ( 50 , 26 ) ; printf ( "related to resizing the window AFTER MAXIMIZING. ");
		
		Sleep ( 2200 ); 	// 2 second delay
		
			gotoXY_STRING ( 67 , 29 ) ; SetConsoleTextAttribute ( hConsole, 11 ) ; printf ( "DESIGN AND CREDIT" );
			gotoXY_STRING ( 37 , 31 ) ; SetConsoleTextAttribute ( hConsole, 15 ) ; printf ( "All of the art related objects belong to their respective owners and sites. " );
			gotoXY_STRING ( 31 , 33 ) ; SetConsoleTextAttribute ( hConsole, 3 ) ; printf ( "The links towards the sources may not be pasted here, as anti-viruses have flagged them " );
			gotoXY_STRING ( 44 , 34 ) ; printf ( "as malicious and may install unwanted files in your computer." );
			gotoXY_STRING ( 31 , 36 ) ; SetConsoleTextAttribute ( hConsole, 15 ) ; printf ( "This is to prevent other people from making the same mistake as one of the coders did." );		//	(yes, I almost installed a virus	~ ajp)
			
		Sleep ( 2200 ); 	// 2 second delay
		
			gotoXY_STRING ( 49 , 40 ) ; SetConsoleTextAttribute ( hConsole, 15 ) ; printf ( "|          Press any key to proceed...         |" );
			WaitKey();
					...
					switch( OPTION_select )
					{
						...
						case 1: 	// if the menu_item 's value is 1, you chose notice
						{
							...
							SetConsoleTextAttribute ( hConsole, 9 ) ; for ( int ASCII_Y_axis = 5 ; ASCII_Y_axis <= 46 ; ASCII_Y_axis++ )
							{
								...
								gotoXY_STRING( 79 , 9 ) ; printf( "      _      ____  _____  _  ____  _____" );
								gotoXY_STRING( 79 , 10 ) ; printf( "     / \\  /|/  _ \\/__ __\\/ \\/   _\\/  __/" );
								gotoXY_STRING( 79 , 11 ) ; printf( "     | |\\ ||| / \\|  / \\  | ||  /  |  \\  " );
								gotoXY_STRING( 79 , 12 ) ; printf( "     | | \\||| \\_/|  | |  | ||  \\__|  /_ " );
								gotoXY_STRING( 79 , 13 ) ; printf( "     \\_/  \\|\\____/  \\_/  \\_/\\____/\\____\\" );
								//gotoXY_STRING( 79 , 14 ) ; printf( "     ===================================" );
